...
### New Features
- **Tile geometry steps carry a sign.** `steps_k` may now be negative: `flip`, `flipud`, `fliplr`, `rot90` and negative-step slices (`da[::-1]`) become lazy rewrites of the tile geometry instead of bounded reads. Engines are unaffected — they keep receiving ascending source selections, and each part is reversed in memory after decoding (@atrabattoni).
