...
- **A coordinate is sorted when its values increase, not when it has no overlaps.** Interpolated and sampled coordinates answered the monotonicity that ordered look-ups depend on by looking for overlaps, which is a different question in both directions. An axis that decreases smoothly — a regular axis running backwards among them — has no discontinuity to report, so it was declared sorted and a `sel` handed it to a binary search that assumes otherwise. Conversely an axis whose overlap is shorter than one sample never stops increasing, yet was declared unsorted, and every slice paid for a split-and-concatenate detour it did not need, with a warning saying so. Both now read their own values (@atrabattoni).
