...
class TestBase:
    ...
    def test_raises_on_data_and_coords_mismatch(self):
        ...
        with pytest.raises(
            ValueError,
            match="inferred number of dimensions 2 from `coords` does not match `data` dimensionality of 1",
        ):
            xd.DataArray(np.zeros(3), coords={"time": [1], "distance": [1]})
...
class TestSelection:
    ...
    def test_sel_slice_with_overlaps(self, ctype):
        ...
        time = time.concat(xd.Coordinate[ctype].from_block(0.0, 10, 0.1, "time"))
        time = time.concat(xd.Coordinate[ctype].from_block(0.5, 10, 0.1, "time"))
        distance = [0.0, 10.0, 20.0, 30.0]
        da = xd.DataArray(data, {"time": time, "distance": distance})

        data = da.values[2:-2]
        time = xd.Coordinate[ctype](None, "time", float)
        time = time.concat(xd.Coordinate[ctype].from_block(0.2, 8, 0.1, "time"))
        time = time.concat(xd.Coordinate[ctype].from_block(0.5, 8, 0.1, "time"))
