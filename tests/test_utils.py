...
class TestCompression:
    def test_compression(self, tmp_path):
        file_path = dascore.utils.downloader.fetch("opto_das_1.hdf5")
        save_path = tmp_path / "opto_das_1_compressed.hdf5"
        ...
        with h5py.File(file_path, 'r') as original_file, h5py.File(save_path, 'r') as compressed_file:

            orig_keys, comp_keys = [], []
            original_file.visit(orig_keys.append)
            compressed_file.visit(comp_keys.append)
            assert set(orig_keys) == set(comp_keys)
